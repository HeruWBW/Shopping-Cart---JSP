...
public class EditUser extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			PreparedStatement st = con.prepareStatement("update user set firstname=?, lastname=?, username=?, password=?, email=?, address=?, phone=?, type=? where id=?");
			...
			response.sendRedirect("http://localhost:8080/Tescrud/admin.jsp");
		}
		catch (Exception e) {
			e.printStackTrace();
		}
