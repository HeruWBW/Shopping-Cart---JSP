...
public class InsertUser extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			PreparedStatement st = con.prepareStatement ("insert into user (username, password, firstname, lastname, email, address, phone, type)"
					+ " values(?, ?, ?, ?, ?, ?, ?, ?)");
			...
			response.sendRedirect("http://localhost:8080/Tescrud/");
		}
		catch (Exception e) {
			e.printStackTrace();
			PrintWriter out = response.getWriter();
